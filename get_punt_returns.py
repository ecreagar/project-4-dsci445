...
def extract_punts_oneyear(filename: str, ids: pd.DataFrame, fileout: str):
    """
    filename:
    ...
    punts = ids.set_index(['gameId', 'playId']).join(tracking_data.set_index(['gameId', 'playId']), how='inner')
    punts.to_csv(fileout)


def combine_punts(punt_files: list, fileout: str, delete: bool = True):
    data = pd.DataFrame()
    for i in punt_files:
